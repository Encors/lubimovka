...
function changeStyleToMobile(event) {
  if(!event.matches && spectacles.length >= 3) {
    sectionSpectacle.classList.add(selectors.sectionSpectacleCountTwoThreeClass);
    titleSpectacle.textContent = 'Заголовок блока с\u00A0тремя\u00A0и\u00A0более спектаклями';
    spectaclesList.addClass(selectors.containerCountThreeClass);
  }
   else if(!event.matches && spectacles.length === 2) {
    sectionSpectacle.classList.add(selectors.sectionSpectacleCountTwoThreeClass);
    titleSpectacle.textContent = 'Заголовок блока\u00A0с\u00A0двумя спектаклями';
    ...
    titleSpectacle.textContent = 'Заголовок блока со\u00A0спектаклями любое количество';
...
function renderSpectaclesList() {
  spectacleItems.forEach((item, index) => {
    ...
    if(spectacleItems[index - 1] && item.getBoundingClientRect().right
      >= spectacleItems[index - 1].getBoundingClientRect().right) {
        item.classList.remove('spectacle__item');
        spectacleItems[index - 1].classList.add('spectacle__item');

      if((spectacleItems[index - 1].getBoundingClientRect().right + 450
